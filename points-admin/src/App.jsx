...
import { ArrayHelpers, RolesHelpers } from "./helpers";
...
function App() {
  ...
  const Points = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const { data } = await AdminAPI.PointsList({
        data: {
          ...filters,
          Pi: pageParam,
          Ps: 12,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (body) => {
      let data = await AdminAPI.PointsDeleteId(body);
  ...
  const onDelete = (item) => {
    ...
    Swal.fire({
      ...
      title: "Xác nhận xoá ?",
      html: `Xoá tích điểm này? Hành động này không thể được hoàn tác.`,
      icon: "warning",
      ...
      preConfirm: async () => {
        const data = await deleteMutation.mutateAsync({ data: dataPost });
        return data;
      },
      ...
      if (result.isConfirmed) {
        ...
        window.top.toastr.success("Xoá thành công.", "", { timeOut: 800 });
      }
    });
  };
  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div id="scrollableDiv" className="overflow-auto grow">
        <InfiniteScroll
          dataLength={Lists.length}
          next={() => Points.fetchNextPage()}
          inverse={false} //
          hasMore={Points.hasNextPage}
                    ...
                    <div className="mb-4 border rounded last:mb-0" key={index}>
                      ...
                      {(moment(item.CreateDate).format("DD-MM-YYYY") ===
                        ...
                        <div className="grid grid-cols-2 gap-3 px-4 py-3.5 border-t">
                          ...
                          >
                            {({ open }) => (
                              <button
                                onClick={open}
                                type="button"
                                className="font-medium text-white rounded shadow-lg h-11 bg-primary"
                              >
                                Chỉnh sửa
                              </button>
                            )}
                          ...
                          <button
                            type="button"
                            className="font-medium text-white rounded shadow-lg h-11 bg-danger"
                ...
                <div className="flex flex-col items-center min-h-full pt-8">
                  ...
                  <div className="mt-6 text-center">
                    ...
                    <div className="mt-1 font-light">
                      Rất tiếc ... Không tìm thấy dữ liệu nào.
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="flex items-center justify-between p-5 text-base border-t">
        <div>
          Tổng
          <span className="px-1.5 font-lato font-bold text-success">
            {(Points?.data?.pages &&
          ...
          >
            {({ open }) => (
              <button
                onClick={open}
                type="button"
                className="h-12 px-5 text-white rounded bg-success"
              >
                Đổi điểm
              </button>
            )}
