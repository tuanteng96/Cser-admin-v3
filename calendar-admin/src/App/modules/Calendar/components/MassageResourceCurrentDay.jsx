...
function MassageResourceCurrentDay({ setInitialValue, onOpenModal }) {
  ...
  const { data, isFetching, isLoading } = useQuery({
    queryKey: ["ListCurrentCalendars", { filters }],
    queryFn: async () => {
      let CrDate = new Date();
      const newFilters = {
        ...filters,
  ...
  return (
    <div className="flex gap-4 lg:gap-8 grow" ref={elementRef}>
      <div
        className="relative flex-1 pt-8 overflow-auto"
                          ...
                          onClick={(e) => {
                            ...
                            if (newPhotoJSON) {
                              Photos = [
                                ...Photos,
                                ...newPhotoJSON.map((x) => ({
                                  src: toAbsoluteUser(x, "/Upload/image/"),
                                })),
                              ];
                            }
      ...
      <div
        className="bg-[#e5e7eb] lg:w-[250px] w-[90px] lg:p-4 p-2 overflow-auto"
        style={{ height: height + "px" }}
      >
        {isLoading && (
          <>
...
export default MassageResourceCurrentDay;
