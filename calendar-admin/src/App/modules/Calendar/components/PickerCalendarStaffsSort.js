...
function PickerCalendarStaffsSort({ children }) {
  ...
  const { isLoading, refetch } = useQuery({
    queryKey: ["StaffSort", AuthCrStockID],
    queryFn: async () => {
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: ({ rowData, rowIndex }) => (
          <div>
            <Controller
              name={`Users[${rowIndex}].source.Order`}
              control={control}
  ...
  const updateMutation = useMutation({
    mutationFn: async (body) => {
      let rs = await CalendarCrud.updateStaffs(body);
      await refetch();
      await queryClient.invalidateQueries({ queryKey: ["CalendarsStaffs"] });
      return rs;
    },
  });
        ...
        createPortal(
          ...
          >
            <div className="flex items-center justify-between px-4 py-3.5 border-b">
              <div className="text-xl font-medium">Danh sách nhân viên</div>
              <div className="flex gap-3">
                <button
                  type="submit"
                  className="rounded-[4px] px-4 text-white bg-primary disabled:opacity-40 flex items-center"
...
export default PickerCalendarStaffsSort;
